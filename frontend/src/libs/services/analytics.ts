...
export type LastPosts = Array<PostInterface>
...
export default class AnalyticsService {
  ...
  static async lastPosts(skip: number = 0, limit: number = 10): Promise<LastPosts> {
    return await callAPI(urls.analytics.lastPosts + `?skip=${skip}&limit=${limit}`, 'GET')
  }
}